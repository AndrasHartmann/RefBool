...
distname={'beta', 'birnbaumsaunders', 'burr', 'exponential', ...
...
%Fit Mixture Model of gaussians with best val
    try
        sortbyindx=find(strcmpi(sortby,sortbyname));
        PD = fitnormalmixture(data,sortbyindx);
        NLL=PD.NegativeLogLikelihood; % -Log(L)
        %If NLL is non-finite number, produce error to ignore distribution
        if ~isfinite(NLL)
            error('non-finite NLL');
        end
        num=length(D)+1;
        PDs(num) = {PD}; %#ok<*AGROW>
        D(num).DistName=PD.DistributionName;
        D(num).NLogL=NLL;
        D(num).BIC=PD.BIC;
        D(num).AIC=PD.AIC;
        D(num).AICc=PD.AIC+((2*2*numel(PD.mu)*(2*numel(PD.mu)+1))/(numel(data)-2*numel(PD.mu)-1));
        D(num).ParamNames={'mu','sigma'};
        D(num).ParamDescription={};
        D(num).Params=[];
        D(num).Paramci=[];
        D(num).ParamCov=[];
...
function PD=fitnormalmixture(data,sortby)
bestval = Inf;
flag = true;
components = 2;
if ~iscolumn(data)
...
while flag
    if ~flag || components > 10
        break
    end
    try
        PD = fitgmdist(data,components,'Replicates',2);
    catch err
        components = components + 1;
        continue;
    end
    AICc = PD.AIC+((2*2*numel(PD.mu)*(2*numel(PD.mu)+1))/(numel(data)-2*numel(PD.mu)-1));
    switch sortby
        case 1
            if PD.NegativeLogLikelihood < bestval
                bestval = PD.NegativeLogLikelihood;
                bestPD = PD;
            else
                flag = false;
            end
